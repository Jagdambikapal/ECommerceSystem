...
namespace ProductService.API.Controllers
{
    ...
    public class ReviewController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update([FromBody] ReviewUpdateDTO updateDto)
        {
            try
            {
                var result = await _reviewService.UpdateAsync(updateDto);
                if (result == null)
                    return NotFound(ApiResponse<string>.FailResponse($"Review with id '{updateDto.Id}' not found"));
